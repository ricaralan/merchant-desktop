...
import com.merchant.components.tableModels.ClienteTableModel;
 ...
 * @author Eleazar
 */
public class ClientesPanel extends MerchantPanel {
    ...
    public ClientesPanel(MerchantConnection c) {
        initComponents();
        this.connection = c.getConnection();
        clienteController = new ClienteController();
        crearCliente = true;
        photo = new Photo();
        photo.setBasePath("/images/clientes/");
    ...
    private MerchantTableModel getTableModel() {
        return ((MerchantTableModel) tableClientes.getModel());
    }
    ...
    private void initComponents() {
        ...
        opcionesCliente = new javax.swing.JPopupMenu();
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Datos Cliente", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, null, new java.awt.Color(102, 153, 255)));
        ...
        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Dirección Cliente", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, null, new java.awt.Color(102, 153, 255)));
        ...
        tableClientes.setFont(new java.awt.Font("Arial", 0, 12)); // NOI18N
        ...
        tableClientes.setComponentPopupMenu(opcionesCliente);
        ...
        tableClientes.getAccessibleContext().setAccessibleName("");
        tableClientes.getAccessibleContext().setAccessibleDescription("");
    ...
    private void EditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_EditarActionPerformed
        ...
        if (row != -1) {
            Cliente cliente = (Cliente) ((ClienteTableModel) tableClientes
                    .getModel())
                    .getObjectByRow(row);
            Domicilio domicilio = new DomicilioController().getById(connection, 
                    cliente.domicilio_id_domicilio);
            setDatosClienteForm(cliente);
            setDatosDomicilioForm(domicilio);
            btnOpcionForm.setText("Actualizar");
            idActualizar = cliente.id_cliente;
            idActualizarDomicilio = cliente.domicilio_id_domicilio;
            crearCliente = false;
        }
    ...
    private void EliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_EliminarActionPerformed
        int row = tableClientes.getSelectedRow();
        if (row != -1) {
            Cliente cliente = (Cliente) ((MerchantTableModel) tableClientes
                    .getModel()).getObjectByRow(row);
            if (cliente.id_cliente == idActualizar) {
                JOptionPane.showMessageDialog(this, "Este registro esta en edición,"
                        + " \n finalize para poder eliminar ", "ERROR AL ELIMINAR", 1);
            } else {
            int res = JOptionPane.showConfirmDialog(this, "¿Esta seguro de eliminar al cliente \""
                    + cliente.nombre + "\"?");
            if (res == JOptionPane.OK_OPTION) {
                if (clienteController.delete(connection, cliente.id_cliente, 
                        cliente.domicilio_id_domicilio)) {
                    ((ClienteTableModel) tableClientes.getModel()).initData(connection);
                } else {
        ...
        if (txtNombre.getText().isEmpty() && txtApellidos.getText().isEmpty()
            ...
            if (validar()) {
                try {
                    ...
                    if (crearCliente) {
                        if (!new ClienteController().create(connection, cliente, domicilio)) {
                            JOptionPane.showMessageDialog(this, "Por favor intente más tarde...",
                                    "ERROR AL REGISTRAR", 1);
                        }
                    } else {
                        if (!new ClienteController().update(connection, cliente, idActualizar,
    ...
    private synchronized Cliente getDatosCliente() {
        Cliente cliente = new Cliente();
        cliente.rfc = txtRFC.getText();
        cliente.nombre = txtNombre.getText();
        cliente.clte_apellidos = txtApellidos.getText();
        cliente.clte_telefono_celular = txtTelCelular.getText();
        cliente.clte_telefono_casa = txtTelCasa.getText();
        cliente.email = txtEmail.getText();
        cliente.domicilio_id_domicilio = idActualizarDomicilio;
        cliente.clte_status = 1;
        cliente.imagen_cliente = photo.getPosiblePathPhoto();
        return cliente;
    }
    ...
    private synchronized void setDatosClienteForm(Cliente cliente) {
        cleanDatosClienteForm();
        txtRFC.setText(cliente.rfc);
        txtNombre.setText(cliente.nombre);
        txtApellidos.setText(cliente.clte_apellidos);
        txtTelCelular.setText(cliente.clte_telefono_celular);
        txtTelCasa.setText(cliente.clte_telefono_casa);
        txtEmail.setText(cliente.email);
        if (cliente.imagen_cliente.length() > 1) {
            photo.setPhoto(cliente.imagen_cliente);
            photo.setPhotoJPanel(panelFoto);
        }
    }
    ...
    private void cleanDatosClienteForm() {
    ...
    public boolean validar() {
        ...
        List<ValidateFieldError> errors = v.validateFields(cliente);
    ...
    private javax.swing.JPopupMenu opcionesCliente;
